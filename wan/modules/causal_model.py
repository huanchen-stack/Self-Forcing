...
class CausalWanSelfAttention(nn.Module):
    ...
    def forward(
        ...
        if profile:
            _end.record()
            torch.cuda.synchronize()
            print(f"\t\tqkv norm: {_start.elapsed_time(_end)} ms")
            torch.cuda.synchronize()
            _start.record()
        ...
        if kv_cache is None:
            ...
            if self._use_cudagraph:
                if self.local_attn_size != -1 and (current_end > self.kv_cache1["global_end_index"]) and (
                        num_new_tokens + self.kv_cache1["local_end_index"] > kv_cache_size):
                    # Calculate the number of new tokens added in this step
                    # Shift existing cache content left to discard oldest tokens
                    # Clone the source slice to avoid overlapping memory error
                    num_evicted_tokens = num_new_tokens + self.kv_cache1["local_end_index"] - kv_cache_size
                    num_rolled_tokens = self.kv_cache1["local_end_index"] - num_evicted_tokens - sink_tokens
                    kv_cache["k"][:, sink_tokens:sink_tokens + num_rolled_tokens] = \
                        kv_cache["k"][:, sink_tokens + num_evicted_tokens:sink_tokens + num_evicted_tokens + num_rolled_tokens].clone()
                    kv_cache["v"][:, sink_tokens:sink_tokens + num_rolled_tokens] = \
                        kv_cache["v"][:, sink_tokens + num_evicted_tokens:sink_tokens + num_evicted_tokens + num_rolled_tokens].clone()
                    # Insert the new keys/values at the end
                    local_end_index = self.kv_cache1["local_end_index"] + current_end - self.kv_cache1["global_end_index"] - num_evicted_tokens
                    local_start_index = local_end_index - num_new_tokens
                    kv_cache["k"][:, local_start_index:local_end_index] = roped_key
                    kv_cache["v"][:, local_start_index:local_end_index] = v
                else:
                    # Assign new keys/values directly up to current_end
                    local_end_index = self.kv_cache1["local_end_index"] + current_end - self.kv_cache1["global_end_index"]
                    local_start_index = local_end_index - num_new_tokens
                    kv_cache["k"][:, local_start_index:local_end_index] = roped_key
                    kv_cache["v"][:, local_start_index:local_end_index] = v
            else:
            ...
            if profile:
                _end.record()
                torch.cuda.synchronize()
                print(f'\t\tattention input size: \n\t\t\t rope: {roped_query.shape} \n\t\t\t k: {kv_cache["k"][:, max(0, local_end_index - self.max_attention_size):local_end_index].shape} \n\t\t\t v: {kv_cache["v"][:, max(0, local_end_index - self.max_attention_size):local_end_index].shape}')
                print(f"\t\tattention: {_start.elapsed_time(_end)} ms")
                torch.cuda.synchronize()
                _start.record()

            if self._use_cudagraph:
                self.kv_cache1["global_end_index"] = current_end
                self.kv_cache1["local_end_index"] = local_end_index
            else:
...
class CausalWanAttentionBlock(nn.Module):
    ...
    def forward(
        ...
        def cross_attn_(x, context, context_lens, crossattn_cache=None):
            x = x + self.cross_attn(self.norm3(x), context,
                                    context_lens, crossattn_cache=crossattn_cache)
            return x
        ...
        x = cross_attn_(x, context, context_lens, crossattn_cache)

        if profile:
            _end.record()
            torch.cuda.synchronize()
            print(f"\tcross-attn: {_start.elapsed_time(_end)} ms")
            torch.cuda.synchronize()
            _start.record()
        ...
        if profile:
            _end.record()
            torch.cuda.synchronize()
            print(f"\tffn: {_start.elapsed_time(_end)} ms")
            torch.cuda.synchronize()
            _start.record()
